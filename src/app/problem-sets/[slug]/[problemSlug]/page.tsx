...
import { Play, Send, Loader2, ChevronRight, Terminal as TerminalIcon, BookOpen, ChevronLeft } from "lucide-react";
...
export default function ProblemSetProblemPage({ params }: { params: Promise<{ slug: string; problemSlug: string }> }) {
    const { slug: setSlug, problemSlug } = use(params);
    const [code, setCode] = useState("");
    const [result, setResult] = useState<any>(null);

    const { data: problem, isLoading } = api.problem.getBySlug.useQuery({ slug: problemSlug });
    const { data: problemSet } = api.problemSet.getBySlug.useQuery({ slug: setSlug });
    ...
    const currentIndex = problemSet?.problems.findIndex(p => p.slug === problemSlug) ?? -1;
    const prevProblem = currentIndex > 0 ? problemSet?.problems[currentIndex - 1] : null;
    const nextProblem = currentIndex >= 0 && currentIndex < (problemSet?.problems.length ?? 0) - 1
                ...
                <div className="h-12 border-b border-white/10 bg-zinc-950 flex items-center justify-between px-4 shrink-0">
                    <div className="flex items-center space-x-3 overflow-hidden">
                        <div className="flex items-center space-x-1 text-zinc-500 text-xs font-medium shrink-0">
                            <Link href="/problem-sets" className="hover:text-green-400 transition-colors">
                                Problem Sets
                            </Link>
                            <ChevronRight className="w-3 h-3" />
                            <Link href={`/problem-sets/${setSlug}`} className="hover:text-green-400 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                        {prevProblem && (
                            <Link href={`/problem-sets/${setSlug}/${prevProblem.slug}`}>
                                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-zinc-400 hover:text-white">
                        ...
                        {nextProblem && (
                            <Link href={`/problem-sets/${setSlug}/${nextProblem.slug}`}>
                                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-zinc-400 hover:text-white">
                                    Next
