...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return createMetadata({
        ...
        description:
            ...
            `Master ${problemSet.title} with ${problemSet.problems.length} hands-on AI/ML coding challenges.`,
        pathname: `/problem-sets/${slug}`,
    });
}

export default async function ProblemSetDetailPage({ params }: PageProps) {
    const { slug } = await params;
    const problemSet = await api.problemSet.getBySlug({ slug });
